...
async function postOrder(user_id, products, pago) {
    ...
    if(pago != undefined &&  user_id != undefined) {
            ...
            const getProducts = products.map(async function(id) {
                ...
                return id});
            const productsList = await Promise.all(getProducts);
            let descripcion = '';
            let monto = 0;
            console.log(productsList);
            for(i=0; i<productsList.length; i++){
                if(productsList[i] === undefined){
                    resultado = 'Producto/s no encontrado/s';
                    break;
                } else {
                    descripcion += '' + products[i].quantity + 'x' + productsList[i].description;
                    monto += productsList[i].product_price * products[i].quantity;
                    const order = await dependencies.sequelize.query(`INSERT INTO orders(estado, hora, descripcion, monto, pago, user_id)
                    VALUES ('${estado}', '${hora}',
                    '${descripcion}', ${monto}, ${pago}, ${user_id})`);
                    resultado = 'Ok';
                }    
            }    
        return resultado;
